                           ...
                           const juce::String& createdAt)
{
    ...
    const char* sql = "INSERT INTO recordings (audio_title, account_name, category, file_path, image_path, price,  created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?);";
    ...
    sqlite3_bind_text(stmt, 7, createdAt.toRawUTF8(), -1, SQLITE_TRANSIENT);
...
std::vector<RecordingEntry> LocalDatabase::getAllRecordings()
{
    ...
    const char* sql = "SELECT id, audio_title, account_name, category, file_path, image_path, price, created_at FROM recordings;";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        entry.price     = (float) sqlite3_column_double(stmt, 6);
        entry.createdAt = juce::String((const char*)sqlite3_column_text(stmt, 7));
